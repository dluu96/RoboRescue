...
def backtrack(): #when colorsensor detects use this to flip 180 after grabbing bottle and continue with leftsearch
	currentGs = gs.value()
	while gs.value() <= currentGs + 170:
		right.run_direct(duty_cycle_sp=-30)
		left.run_direct(duty_cycle_sp=30)
...
while True:
	try:
		...
		if ts.value():
			...
			backup()
		if us.value() > 200:
		#when wall outside of range turn left 90 degrees
			turnLeft()
			moveForward()
			print 3
	except Found as t:
